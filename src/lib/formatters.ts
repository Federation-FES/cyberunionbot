 ...
 */
export function formatPrice(kopecks: number | string | null | undefined): string {
 // Преобразуем в число, если пришло как строка
 const numKopecks = typeof kopecks === 'string' 
   ? parseFloat(kopecks) 
   : (typeof kopecks === 'number' ? kopecks : 0);
 
 // Проверяем валидность
 if (isNaN(numKopecks) || numKopecks < 0) {
 console.warn('Invalid price value:', kopecks);
 return '0 ₽';
 }
 
 const rubles = numKopecks / 100;
